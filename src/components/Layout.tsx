...
import { getMonthDisplayName, getMonthPickerRange, shiftMonth } from '../utils/helpers';
...
const navItems = [
  ...
  { id: 'expenses', label: 'Ausgaben', icon: Wallet },
  { id: 'budget', label: 'Budgets', icon: Target },
  { id: 'savings', label: 'Sparziele', icon: PiggyBank },
  { id: 'accounts', label: 'Konten', icon: Landmark },
  { id: 'banksync', label: 'Bank Sync', icon: RefreshCw },
  { id: 'freelance', label: 'Freelance', icon: CalendarDays },
  ...
  return (
    <div className="app-shell flex h-screen bg-slate-50 dark:bg-gray-950 overflow-hidden">
      ...
      >
        ...
        {/* Theme Toggle */}
        <div className="p-3 border-t border-slate-200 dark:border-gray-800 flex-shrink-0">
          <button
            onClick={cycleTheme}
            className={`
      ...
      {mobileMenuOpen && (
        <>
          <div
            className="lg:hidden fixed inset-0 z-40 bg-black/40 animate-overlay"
      ...
      {currentPage !== 'settings' && <QuickCaptureFab />}
    </div>
  );
}
