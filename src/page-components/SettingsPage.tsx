...
import { Bot, Check, Database, Download, Eye, EyeOff, LoaderCircle, Mic, PlugZap, ReceiptText, RotateCcw, ShieldCheck, Upload } from 'lucide-react';
...
import { inspectGoogleDriveAuthSetup, listGoogleDriveBackups, performGoogleDriveBackup, performGoogleDriveLiveSync, restoreGoogleDriveBackup } from '../services/googleDriveBackup';
import { connectLocalBackupDirectory, isLocalFolderBackupSupported, listLocalFolderBackups, performLocalFolderBackup, performLocalFolderLiveSync, restoreLocalFolderBackup } from '../services/localFolderBackup';
import type { FinanceState, Settings } from '../types';

const STORAGE_LABELS: Record<Settings['backupProvider'], string> = {
  device: 'Gerätespeicher',
...
export function SettingsPage() {
  ...
  const [driveStatusTone, setDriveStatusTone] = useState<'success' | 'error'>('success');
  const [driveLoading, setDriveLoading] = useState<'connect' | 'backup' | 'list' | 'restore' | null>(null);
  const [driveBackups, setDriveBackups] = useState<Array<{ id: string; name: string; createdTime: string; size?: number }>>([]);
  const [localFolderStatus, setLocalFolderStatus] = useState('');
  const [localFolderStatusTone, setLocalFolderStatusTone] = useState<'success' | 'error'>('success');
  const [localFolderLoading, setLocalFolderLoading] = useState<'connect' | 'backup' | 'list' | 'restore' | null>(null);
  const [localFolderBackups, setLocalFolderBackups] = useState<Array<{ name: string; createdTime: string; size?: number }>>([]);
  const localFolderSupported = useMemo(() => isLocalFolderBackupSupported(), []);
  ...
  const setDriveMessage = (message: string, tone: 'success' | 'error' = 'success') => {
    setDriveStatus(message);
    setDriveStatusTone(tone);
  };

  const setLocalFolderMessage = (message: string, tone: 'success' | 'error' = 'success') => {
    setLocalFolderStatus(message);
    setLocalFolderStatusTone(tone);
  };
  ...
  const syncGoogleDriveFolder = (folderId: string) => {
    updateSetting('googleDrive', {
      ...settings.googleDrive,
      folderId,
    });
  };

  const ensureGoogleDriveAuthReady = async () => {
    const diagnostics = await inspectGoogleDriveAuthSetup(settings.googleDrive.clientId);
    if (diagnostics.issues.length > 0) {
      setDriveMessage(diagnostics.issues[0], 'error');
      return false;
    }
  ...
  const handleConnectGoogleDrive = async () => {
    if (!settings.googleDrive.clientId.trim()) {
      setDriveMessage('Bitte zuerst die Google Client-ID eintragen.', 'error');
      return;
    }

    if (!(await ensureGoogleDriveAuthReady())) {
      return;
    }

    setDriveLoading('connect');
    setDriveStatus('');
    try {
      const result = await listGoogleDriveBackups(settings.googleDrive.clientId, settings.googleDrive.folderId || undefined, true);
      const nextGoogleDriveSettings = {
        ...settings.googleDrive,
        folderId: result.folderId,
      };

      if (settings.backupFrequency === 'live') {
        const liveSync = await performGoogleDriveLiveSync(state, {
          clientId: settings.googleDrive.clientId,
          folderId: result.folderId,
          fileId: settings.googleDrive.liveSyncFileId || undefined,
          interactive: true,
        });

        updateSetting('googleDrive', {
          ...nextGoogleDriveSettings,
          liveSyncFileId: liveSync.fileId,
          liveSyncFileName: liveSync.fileName,
          lastLiveSyncAt: liveSync.timestamp,
        });
      } else {
        updateSetting('googleDrive', nextGoogleDriveSettings);
      }

      setDriveBackups(result.backups);
      setDriveMessage(settings.backupFrequency === 'live' ? 'Google Drive verbunden und Live-Datei angelegt.' : 'Google Drive verbunden.');
    } catch (error) {
      setDriveMessage(error instanceof Error ? error.message : 'Google Drive konnte nicht verbunden werden.', 'error');
    } finally {
  ...
  const handleGoogleDriveBackup = async () => {
    ...
    setDriveLoading('backup');
    setDriveStatus('');
    try {
      const result = settings.backupFrequency === 'live'
        ? await performGoogleDriveLiveSync(state, {
            clientId: settings.googleDrive.clientId,
      ...
      const refreshed = await listGoogleDriveBackups(settings.googleDrive.clientId, result.folderId, false);
      setDriveBackups(refreshed.backups);
      flash(settings.backupFrequency === 'live' ? 'Live-Sicherung nach Google Drive geschrieben' : 'Backup in Google Drive gespeichert');
      setDriveMessage(settings.backupFrequency === 'live' ? 'Live-Sicherung erfolgreich aktualisiert.' : 'Backup erfolgreich in Google Drive gespeichert.');
    } catch (error) {
      setDriveMessage(error instanceof Error ? error.message : 'Backup in Google Drive fehlgeschlagen.', 'error');
    } finally {
      setDriveLoading(null);
    }
  };

  const handleLoadGoogleDriveBackups = async () => {
    if (!settings.googleDrive.clientId.trim()) {
      setDriveMessage('Bitte zuerst die Google Client-ID eintragen.', 'error');
      return;
    }
    ...
    try {
      const result = await listGoogleDriveBackups(settings.googleDrive.clientId, settings.googleDrive.folderId || undefined, true);
      syncGoogleDriveFolder(result.folderId);
      setDriveBackups(result.backups);
      setDriveMessage(`${result.backups.length} Backup${result.backups.length === 1 ? '' : 's'} geladen.`);
    } catch (error) {
      setDriveMessage(error instanceof Error ? error.message : 'Backups konnten nicht geladen werden.', 'error');
    } finally {
      setDriveLoading(null);
    }
  };

  const handleRestoreGoogleDriveBackup = async (fileId: string, fileName: string) => {
    if (!settings.googleDrive.clientId.trim()) {
      setDriveMessage('Bitte zuerst die Google Client-ID eintragen.', 'error');
      return;
    }

    if (!confirm(`Backup "${fileName}" wirklich wiederherstellen? Der aktuelle App-Stand wird ersetzt.`)) {
      return;
    }
    ...
    setDriveStatus('');
    try {
      const parsed = await restoreGoogleDriveBackup(settings.googleDrive.clientId, fileId, true);
      dispatch({
        type: 'IMPORT_DATA',
        payload: {
          ...parsed,
          settings: {
            ...
            googleDrive: {
              clientId: settings.googleDrive.clientId,
              folderId: settings.googleDrive.folderId || parsed.settings?.googleDrive?.folderId,
            ...
            localFolder: {
              ...
              ...parsed.settings?.localFolder,
              directoryName: settings.localFolder.directoryName || parsed.settings?.localFolder?.directoryName,
            },
            notifications: {
      ...
      flash('Backup aus Google Drive wiederhergestellt');
      setDriveMessage('Backup wurde erfolgreich wiederhergestellt.');
    } catch (error) {
      setDriveMessage(error instanceof Error ? error.message : 'Backup konnte nicht wiederhergestellt werden.', 'error');
    } finally {
      setDriveLoading(null);
    }
  };

  const syncLocalFolder = (directoryName: string) => {
    updateSetting('localFolder', {
      ...settings.localFolder,
      directoryName,
    });
  };

  const ensureLocalFolderReady = () => {
    if (!localFolderSupported) {
      setLocalFolderMessage('Lokale Live-Backups funktionieren in diesem Browser nicht. Bitte Chrome, Edge oder einen kompatiblen Browser verwenden.', 'error');
      return false;
    }
  ...
  const handleConnectLocalFolder = async () => {
    if (!ensureLocalFolderReady()) {
      return;
    }

    setLocalFolderLoading('connect');
    setLocalFolderStatus('');
    try {
      const connected = await connectLocalBackupDirectory();
      const nextLocalFolderSettings = {
        ...settings.localFolder,
        directoryName: connected.directoryName,
      };

      if (settings.backupFrequency === 'live') {
        const liveSync = await performLocalFolderLiveSync(state, true);
        updateSetting('localFolder', {
          ...nextLocalFolderSettings,
          liveSyncFileName: liveSync.fileName,
          lastLiveSyncAt: liveSync.timestamp,
        });
      } else {
        updateSetting('localFolder', nextLocalFolderSettings);
      }

      const refreshed = await listLocalFolderBackups(false);
      setLocalFolderBackups(refreshed.backups);
      setLocalFolderMessage(settings.backupFrequency === 'live'
        ? 'Lokaler Ordner verbunden und Live-Datei angelegt.'
        : 'Lokaler Backup-Ordner verbunden.');
    } catch (error) {
      setLocalFolderMessage(error instanceof Error ? error.message : 'Lokaler Backup-Ordner konnte nicht verbunden werden.', 'error');
    } finally {
      setLocalFolderLoading(null);
    }
  };
  ...
  const handleLocalFolderBackup = async () => {
    ...
    setLocalFolderLoading('backup');
    setLocalFolderStatus('');
    try {
      const result = settings.backupFrequency === 'live'
        ? await performLocalFolderLiveSync(state, true)
        : await performLocalFolderBackup(state, true);
      ...
      const refreshed = await listLocalFolderBackups(false);
      setLocalFolderBackups(refreshed.backups);
      flash(settings.backupFrequency === 'live' ? 'Live-Sicherung in lokalen Ordner geschrieben' : 'Backup im lokalen Ordner gespeichert');
      setLocalFolderMessage(settings.backupFrequency === 'live' ? 'Live-Sicherung im lokalen Ordner aktualisiert.' : 'Backup erfolgreich im lokalen Ordner gespeichert.');
    } catch (error) {
      setLocalFolderMessage(error instanceof Error ? error.message : 'Backup im lokalen Ordner fehlgeschlagen.', 'error');
    } finally {
      setLocalFolderLoading(null);
    }
  };

  const handleLoadLocalFolderBackups = async () => {
    if (!ensureLocalFolderReady()) {
      return;
    }

    setLocalFolderLoading('list');
    setLocalFolderStatus('');
    try {
      const result = await listLocalFolderBackups(true);
      syncLocalFolder(result.directoryName);
      setLocalFolderBackups(result.backups);
      setLocalFolderMessage(`${result.backups.length} Backup${result.backups.length === 1 ? '' : 's'} aus dem lokalen Ordner geladen.`);
    } catch (error) {
      setLocalFolderMessage(error instanceof Error ? error.message : 'Lokale Backups konnten nicht geladen werden.', 'error');
    } finally {
      setLocalFolderLoading(null);
    }
  };

  const handleRestoreLocalFolderBackup = async (fileName: string) => {
    if (!ensureLocalFolderReady()) {
      return;
    }
    ...
    setLocalFolderLoading('restore');
    setLocalFolderStatus('');
    try {
      const parsed = await restoreLocalFolderBackup(fileName, true);
      dispatch({
        type: 'IMPORT_DATA',
        payload: {
          ...parsed,
      ...
      flash('Backup aus lokalem Ordner wiederhergestellt');
      setLocalFolderMessage('Backup wurde erfolgreich aus dem lokalen Ordner wiederhergestellt.');
    } catch (error) {
      setLocalFolderMessage(error instanceof Error ? error.message : 'Backup konnte nicht aus dem lokalen Ordner wiederhergestellt werden.', 'error');
    } finally {
      setLocalFolderLoading(null);
    }
  };
  ...
  return (
    <div className="max-w-6xl space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="p-5">
          <h3 className="mb-4 flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
            <Database size={16} className="text-amber-500" /> Speicher & Cloud-Backups
          </h3>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Select label="Speichermodus" value={settings.storageMode} onChange={(value) => updateSetting('storageMode', value as Settings['storageMode'])} options={[
              { value: 'local', label: 'Lokal im Browser' },
              { value: 'hybrid', label: 'Hybrid: lokal + Export' },
              { value: 'cloud-export', label: 'Cloud-ready Export' },
            ]} />
            <Select label="Backup-Ziel" value={settings.backupProvider} onChange={(value) => updateSetting('backupProvider', value as Settings['backupProvider'])} options={Object.entries(STORAGE_LABELS).map(([value, label]) => ({ value, label }))} />
            <Select label="Backup-Rhythmus" value={settings.backupFrequency} onChange={(value) => updateSetting('backupFrequency', value as Settings['backupFrequency'])} options={[
              { value: 'manual', label: 'Manuell' },
              { value: 'live', label: 'Live / bei Änderungen' },
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            {[
              { title: 'Lokaler Ordner', text: 'Direkt in einen ausgewaehlten Ordner schreiben - auch im Live-Modus.', active: settings.backupProvider === 'local-folder' },
              { title: 'iCloud Drive', text: 'Ideal für Apple-Geräte und lokale Backups.', active: settings.backupProvider === 'icloud' },
              { title: 'Google Drive', text: 'Gut für Web- und Android-Workflows.', active: settings.backupProvider === 'google-drive' },
              { title: 'Dropbox', text: 'Einfach für universelle Datei-Synchronisierung.', active: settings.backupProvider === 'dropbox' },
              { title: 'OneDrive', text: 'Stark für Windows- und Office-Umgebungen.', active: settings.backupProvider === 'onedrive' },
            ].map((provider) => (
              <div key={provider.title} className={`rounded-2xl border p-4 ${provider.active ? 'border-blue-300 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/20' : 'border-slate-200 bg-slate-50/70 dark:border-gray-800 dark:bg-gray-900'}`}>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">{provider.title}</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-gray-500">{provider.text}</p>
              </div>
            ))}
          </div>
          {settings.backupProvider === 'local-folder' && (
            <div className="mt-4 space-y-4 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 dark:border-emerald-900/40 dark:bg-emerald-950/20">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">Lokalen Backup-Ordner aktivieren</p>
                  <p className="mt-1 text-xs text-slate-600 dark:text-gray-400">
                    Waehle einen Ordner auf deinem PC. Danach kann die App manuelle Backups und eine feste Live-Datei direkt dort aktualisieren.
                  </p>
                </div>
                {(settings.localFolder.lastBackupAt || settings.localFolder.lastLiveSyncAt) && (
                  <div className="rounded-xl bg-white/80 px-3 py-2 text-xs text-slate-600 dark:bg-gray-900/60 dark:text-gray-300">
                    {settings.backupFrequency === 'live' && settings.localFolder.lastLiveSyncAt
                      ? `Letzte Live-Sicherung: ${formatDate(settings.localFolder.lastLiveSyncAt)}${settings.localFolder.liveSyncFileName ? ` · ${settings.localFolder.liveSyncFileName}` : ''}`
                      : `Letztes Backup: ${formatDate(settings.localFolder.lastBackupAt || '')}${settings.localFolder.lastBackupFileName ? ` · ${settings.localFolder.lastBackupFileName}` : ''}`}
                  </div>
                )}
              </div>

              <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-4 text-sm dark:border-gray-700 dark:bg-gray-900/40">
                <p className="font-semibold text-gray-900 dark:text-white">Ordner-Freigabe im Browser</p>
                <p className="mt-2 text-xs text-slate-500 dark:text-gray-500">
                  Die App merkt sich den ausgewaehlten Ordner lokal im Browser und schreibt dann direkt in diesen Ordner. Falls die Freigabe spaeter verloren geht, einfach den Ordner erneut verbinden.
                </p>
              </div>

              {!localFolderSupported && (
                <div className="rounded-2xl border border-red-200 bg-red-50/80 p-4 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-300">
                  Dieser Browser unterstuetzt keine lokale Ordner-Synchronisierung. Bitte Chrome, Edge oder einen kompatiblen Browser verwenden.
                </div>
              )}

              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <Button variant="secondary" onClick={handleConnectLocalFolder} disabled={!localFolderSupported || localFolderLoading !== null}>
                  {localFolderLoading === 'connect' ? <LoaderCircle size={14} className="animate-spin" /> : <PlugZap size={14} />}
                  Ordner waehlen
                </Button>
                <Button onClick={handleLocalFolderBackup} disabled={!localFolderSupported || localFolderLoading !== null}>
                  {localFolderLoading === 'backup' ? <LoaderCircle size={14} className="animate-spin" /> : <Upload size={14} />}
                  Jetzt sichern
                </Button>
                <Button variant="secondary" onClick={handleLoadLocalFolderBackups} disabled={!localFolderSupported || localFolderLoading !== null}>
                  {localFolderLoading === 'list' ? <LoaderCircle size={14} className="animate-spin" /> : <Download size={14} />}
                  Backups laden
                </Button>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <div className="rounded-2xl bg-white/80 p-3 dark:bg-gray-900/60">
                  <p className="text-xs text-slate-500 dark:text-gray-500">Auto-Backup</p>
                  <p className="mt-1 text-sm font-semibold text-gray-900 dark:text-white">
                    {settings.backupFrequency === 'manual'
                      ? 'Manuell'
                      : settings.backupFrequency === 'live'
                        ? 'Live aktiv'
                        : `${settings.backupFrequency === 'weekly' ? 'Woechentlich' : 'Monatlich'} aktiv`}
                  </p>
                </div>
                <div className="rounded-2xl bg-white/80 p-3 dark:bg-gray-900/60">
                  <p className="text-xs text-slate-500 dark:text-gray-500">Lokaler Ordner</p>
                  <p className="mt-1 truncate text-sm font-semibold text-gray-900 dark:text-white">{settings.localFolder.directoryName || 'Noch kein Ordner verbunden'}</p>
                </div>
                <div className="rounded-2xl bg-white/80 p-3 dark:bg-gray-900/60">
                  <p className="text-xs text-slate-500 dark:text-gray-500">Live-Datei</p>
                  <p className="mt-1 truncate text-sm font-semibold text-gray-900 dark:text-white">{settings.localFolder.liveSyncFileName || 'Wird im Live-Modus automatisch angelegt'}</p>
                </div>
              </div>

              {settings.backupFrequency === 'live' && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4 text-sm text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-300">
                  Aenderungen werden nach kurzer Verzoegerung automatisch in eine feste Datei im ausgewaehlten Ordner geschrieben. Das verhaelt sich wie ein lokaler Live-Sync.
                </div>
              )}

              {localFolderStatus && (
                <div className={`rounded-2xl px-4 py-3 text-sm font-medium ${
                  localFolderStatusTone === 'success'
              ...
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-gray-500">Lokale Backups</p>
                {localFolderBackups.length > 0 ? (
                  <div className="space-y-2">
                    {localFolderBackups.map((backup) => (
                      <div key={backup.name} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900 md:flex-row md:items-center md:justify-between">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold text-gray-900 dark:text-white">{backup.name}</p>
                          <p className="mt-1 text-xs text-slate-500 dark:text-gray-500">
                            {formatDate(backup.createdTime)} · {backup.size ? `${(backup.size / 1024).toFixed(1)} KB` : 'Dateigroesse unbekannt'}
                          </p>
                        </div>
                        <Button variant="secondary" size="sm" onClick={() => handleRestoreLocalFolderBackup(backup.name)} disabled={localFolderLoading !== null}>
                          {localFolderLoading === 'restore' ? <LoaderCircle size={14} className="animate-spin" /> : <RotateCcw size={14} />}
                          Wiederherstellen
                        </Button>
                      </div>
                    ))}
                  ...
                  <p className="text-sm text-slate-500 dark:text-gray-500">Noch keine lokalen Backups geladen.</p>
                )}
              </div>
            </div>
          )}
          {settings.backupProvider === 'google-drive' && (
            <div className="mt-4 space-y-4 rounded-2xl border border-blue-200 bg-blue-50/60 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">Google Drive aktivieren</p>
                  <p className="mt-1 text-xs text-slate-600 dark:text-gray-400">
                    Trage die OAuth Client-ID deiner Google Cloud Web-App ein. Danach kannst du verbinden, sichern und Backups direkt wiederherstellen.
                  </p>
                </div>
                {(settings.googleDrive.lastBackupAt || settings.googleDrive.lastLiveSyncAt) && (
                  <div className="rounded-xl bg-white/80 px-3 py-2 text-xs text-slate-600 dark:bg-gray-900/60 dark:text-gray-300">
                    {settings.backupFrequency === 'live' && settings.googleDrive.lastLiveSyncAt
                      ? `Letzte Live-Sicherung: ${formatDate(settings.googleDrive.lastLiveSyncAt)}${settings.googleDrive.liveSyncFileName ? ` · ${settings.googleDrive.liveSyncFileName}` : ''}`
                      : `Letztes Backup: ${formatDate(settings.googleDrive.lastBackupAt || '')}${settings.googleDrive.lastBackupFileName ? ` · ${settings.googleDrive.lastBackupFileName}` : ''}`}
                  </div>
                )}
              </div>
              ...
              <Input
                ...
                onChange={(value) => updateSetting('googleDrive', { ...settings.googleDrive, clientId: value })}
                placeholder="z.B. 1234567890-abc123def456.apps.googleusercontent.com"
              />

              <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-4 text-sm dark:border-gray-700 dark:bg-gray-900/40">
                <p className="font-semibold text-gray-900 dark:text-white">Diese laufende App-URL muss in Google freigegeben sein</p>
                <p className="mt-2 break-all font-mono text-blue-700 dark:text-blue-300">{currentOrigin}</p>
                <p className="mt-2 text-xs text-slate-500 dark:text-gray-500">
                  Trage genau diese Origin bei den autorisierten JavaScript-Quellen ein. Falls Google weiter zickt, dieselbe URL zusätzlich auch bei den autorisierten Weiterleitungs-URIs eintragen.
                </p>
              </div>

              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <Button variant="secondary" onClick={handleConnectGoogleDrive} disabled={driveLoading !== null}>
                  {driveLoading === 'connect' ? <LoaderCircle size={14} className="animate-spin" /> : <PlugZap size={14} />}
                  Verbinden
                </Button>
                <Button onClick={handleGoogleDriveBackup} disabled={driveLoading !== null}>
                  {driveLoading === 'backup' ? <LoaderCircle size={14} className="animate-spin" /> : <Upload size={14} />}
                  Jetzt sichern
                </Button>
                <Button variant="secondary" onClick={handleLoadGoogleDriveBackups} disabled={driveLoading !== null}>
                  {driveLoading === 'list' ? <LoaderCircle size={14} className="animate-spin" /> : <Download size={14} />}
                  Backups laden
                </Button>
              </div>
                ...
                <div className="rounded-2xl bg-white/80 p-3 dark:bg-gray-900/60">
                  ...
                  <p className="mt-1 text-sm font-semibold text-gray-900 dark:text-white">
                    {settings.backupFrequency === 'manual'
                      ...
                      : settings.backupFrequency === 'live'
                        ? 'Live aktiv'
                        : `${settings.backupFrequency === 'weekly' ? 'Wöchentlich' : 'Monatlich'} aktiv`}
                  </p>
                </div>
                <div className="rounded-2xl bg-white/80 p-3 dark:bg-gray-900/60">
                  <p className="text-xs text-slate-500 dark:text-gray-500">Drive-Ordner</p>
                  <p className="mt-1 truncate text-sm font-semibold text-gray-900 dark:text-white">{settings.googleDrive.folderId || 'Wird beim ersten Backup angelegt'}</p>
                </div>
                <div className="rounded-2xl bg-white/80 p-3 dark:bg-gray-900/60">
                  <p className="text-xs text-slate-500 dark:text-gray-500">Live-Datei</p>
                  <p className="mt-1 truncate text-sm font-semibold text-gray-900 dark:text-white">{settings.googleDrive.liveSyncFileName || 'Wird im Live-Modus automatisch angelegt'}</p>
                </div>
              </div>

              {settings.backupFrequency === 'live' && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4 text-sm text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-300">
                  Änderungen werden nach kurzer Verzögerung automatisch in eine feste Drive-Datei geschrieben. Das fühlt sich wie eine laufende Cloud-Sicherung an, bleibt aber weiterhin lokal-first.
                </div>
              )}

              {driveStatus && (
                <div className={`rounded-2xl px-4 py-3 text-sm font-medium ${
                  driveStatusTone === 'success'
                    ? 'border border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-400'
                    : 'border border-red-200 bg-red-50 text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300'
                }`}>
                  {driveStatus}
              ...
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-gray-500">Drive-Backups</p>
                {driveBackups.length > 0 ? (
                  <div className="space-y-2">
                    {driveBackups.map((backup) => (
                      <div key={backup.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900 md:flex-row md:items-center md:justify-between">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold text-gray-900 dark:text-white">{backup.name}</p>
                          <p className="mt-1 text-xs text-slate-500 dark:text-gray-500">
                            {formatDate(backup.createdTime)} · {backup.size ? `${(backup.size / 1024).toFixed(1)} KB` : 'Dateigröße unbekannt'}
                          </p>
                        </div>
                        <Button variant="secondary" size="sm" onClick={() => handleRestoreGoogleDriveBackup(backup.id, backup.name)} disabled={driveLoading !== null}>
                          {driveLoading === 'restore' ? <LoaderCircle size={14} className="animate-spin" /> : <RotateCcw size={14} />}
                          Wiederherstellen
                        </Button>
                      </div>
                    ))}
                  ...
                  <p className="text-sm text-slate-500 dark:text-gray-500">Noch keine Google-Drive-Backups geladen.</p>
                )}
              </div>
            </div>
          )}
